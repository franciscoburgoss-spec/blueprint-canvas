    ...
    (set, get) => ({
      ...
      deleteDocument: (docName) => {
        ...
        set((state) => {
          ...
          const docStillExists = activeProject?.documents.some(d => d.name === docName) === false;
          const wasActiveDocDeleted = docStillExists;

          return {
            projects: updatedProjects,
            activeDocumentId: wasActiveDocDeleted ? null : state.activeDocumentId,
            annotations: [
              ...state.annotations,
              {
                id: Date.now(),
              ...
              return {
                ...
                documents: project.documents.map((doc) => {
                  if (doc.name === docName) {
                    ...
                    return {
                      ...doc,
                      observations: [...doc.observations, newObs],
                    };
                  }
                  return doc;
                }),
            ...
            if (project.id === activeProjectId) {
              return {
                ...
                documents: project.documents.map((doc) => ({
                  ...doc,
                  observations: doc.observations.map((obs) =>
                    obs.id === obsId ? { ...obs, status: 'aprobada' } : obs
                  ),
                })),
              };
            }
            ...
            if (project.id === activeProjectId) {
              return {
                ...
                documents: project.documents.map((doc) => ({
                  ...doc,
                  observations: doc.observations.map((obs) =>
                    obs.id === obsId ? { ...obs, status: 'rechazada' } : obs
                  ),
                })),
              };
            }
          ...
          return {
            ...
            annotations: [
              ...state.annotations,
              {
                id: Date.now(),
                type: 'info',
                message: `[INFO] Nota agregada al panel derecho`,
                timestamp: Date.now(),
              }
            ],
            timeline: [
              ...state.timeline,
              {
                id: generateTimelineId(),
                action: 'ADD_NOTE',
                details: `Nota agregada: "${text.substring(0, 30)}..."`,
                timestamp: Date.now(),
              }
            ],
          };
      ...
      searchObservations: (query) => {
        ...
        project.documents.forEach(doc => {
          doc.observations.forEach(obs => {
            if (obs.text.toLowerCase().includes(query.toLowerCase()) ||
        ...
        set((state) => ({
          ...
          annotations: [
            ...state.annotations,
            {
              id: Date.now(),
              type: 'info',
              message: `[INFO] Búsqueda: "${query}" - ${results.length} resultado(s) encontrado(s)`,
              timestamp: Date.now(),
            }
          ],
        }));
      ...
      clearHistory: () => {
        set({ commandHistory: [], annotations: [] });
      },
